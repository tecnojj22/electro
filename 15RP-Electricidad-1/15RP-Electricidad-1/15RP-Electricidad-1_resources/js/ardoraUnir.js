...
$("#"+sel1).css({ background: colorBack, borderColor: colorLeave });sel1="";drawJoin(0,0);drawGame();}}});
...
$(this).css("background","radial-gradient(ellipse at center, "+color1+", "+color2+")");
$(this).css("filter","progid:DXImageTransform.Microsoft.gradient(startColorstr='"+color1+"', endColorstr='"+color2+"')");
});$(".conten").mouseleave(function () {if ($(this).attr("id")!=sel1){ $(this).css({ background: colorBack, borderColor: colorLeave });}});
...
$(".conten").keydown(function(e){if (e.which!=9 && e.which!=27){if (sel1.localeCompare("") == 0) {sel1 = $(this).attr("id");$(this).css({background: colorSele,borderColor: colorLeave});var color2=colorSele;var color1=colorBack;$(this).css("background", "-webkit-radial-gradient(center, ellipse, " + color1 + "," + color2 + ")");
...
for (i = 0; i < con3.length; i++) {var ind = Math.floor(Math.random()*(con3.length-1))+1;var divId="conten3_"+parseInt(ind);
while (document.getElementById(divId).innerHTML.localeCompare("")!=0){ind++;if (ind>con3.length){ind=1;}divId="conten3_"+parseInt(ind);}document.getElementById(divId).innerHTML=con3[i];}
for (i = 0; i < con4.length; i++) {var ind = Math.floor(Math.random()*(con4.length-1))+1;var divId="conten4_"+parseInt(ind);
while (document.getElementById(divId).innerHTML.localeCompare("")!=0){ind++;if (ind>con4.length){ind=1;}divId="conten4_"+parseInt(ind);}document.getElementById(divId).innerHTML=con4[i];}
for (i = 0; i < con5.length; i++) {var ind = Math.floor(Math.random()*(con5.length-1))+1;var divId="conten5_"+parseInt(ind);
while (document.getElementById(divId).innerHTML.localeCompare("")!=0){ind++;if (ind>con5.length){ind=1;}divId="conten5_"+parseInt(ind);}document.getElementById(divId).innerHTML=con5[i];}
}
function isCorrect(){
